...
<form style="display: none;" action="<%= portletURL.toString() %>" method="get" name="<portlet:namespace />fm">
	...
	<div class="search">
		<input class="search-input" id="<portlet:namespace />name" 
		name="<portlet:namespace />name" placeholder="<liferay-ui:message key="go-to" />" 
		size="30" type="text" value="<%= HtmlUtil.escape(name) %>" />
	...
	if (sitesTabsContainer && siteList) {
		...
		siteList.delegate(
			...
			function(event) {
				...
				<liferay-portlet:renderURL var="viewSitesURL1" windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>">
					<portlet:param name="mvcPath" value="/sites/view_sites.jsp" />
				</liferay-portlet:renderURL>

				Liferay.SO.Sites.displayPopup('<%= viewSitesURL1 %>', '<liferay-ui:message key="sites" unicode="<%= true %>" />', data);
			},
			'.more a'
		);
