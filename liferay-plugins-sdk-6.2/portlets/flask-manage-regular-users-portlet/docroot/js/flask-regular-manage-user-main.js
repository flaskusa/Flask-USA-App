...
function contextMenuHandler(menuItemText, rowData){
	...
	case "Delete" : 
		...
		break;
	case "Change role" :
		setFlaskRoles('roleId', rowData.roleId);
		break;
	}*/
};
...
function deleteAdmin(adminId) {
	...
	var flaskRequest = new Request();
	flaskRequest.sendPOSTRequest(_userModel.SERVICE_ENDPOINTS.DELETE_USERS, param, 
					function (data){
...
function editAdmin(rowData) {
	_flaskLib.loadDataToForm("adminForm",  _userModel.DATA_MODEL.USER, rowData, function(){	
	});
	$("#adminDataTable").hide();
	adminForm.show();
...
function saveAdmin(){
	params = _flaskLib.getFormData('adminForm',_userModel.DATA_MODEL.USER,
...
function fnUpdateProfilePic(uid){
	...
	function setFlaskRoles(elementId, selectedId){
		var param = {};
		var request = new Request();
		var selectList = $('#' + elementId);
		var flaskRequest = new Request();
		flaskRequest.sendGETRequest(_userModel.SERVICE_ENDPOINTS.SET_ROLES, param, 
						...
						function (data){
							console.log("Error in Setting role list: " + data );
						});
